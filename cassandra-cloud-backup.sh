...
function copy_to_gcs() {
  ...
  if ${DRY_RUN}; then
    ...
    if ${SPLIT_FILE}; then
      ...
      nice -n 19 time ${GSUTIL} -m -o 'GSUtil:parallel_composite_upload_threshold=150M' -o 'GSUtil:parallel_process_count:7' cp "${COMPRESS_DIR}/${ARCHIVE_FILE}" "${GCS_BACKUP_PATH}"
    fi
  fi
}
